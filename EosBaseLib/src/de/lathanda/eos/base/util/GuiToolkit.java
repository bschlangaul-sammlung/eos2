...
public class GuiToolkit {
	...
	public static void setScreenResolution(int dpi) {
		if (dpi == 0) {
			GuiToolkit.dpi = Toolkit.getDefaultToolkit().getScreenResolution();
		} else {
			GuiToolkit.dpi = dpi;
		}
	}
	...
	public static JButton createButton(Image image, String tooltip, ActionListener action) {
		JButton btn = new JButton();
		ImageIcon icon = null;
		icon = new ImageIcon(image.getScaledInstance(dpi * 40 / 96, dpi * 40 / 96, Image.SCALE_SMOOTH));
	...
	public static <T> JComboBox<T> createComboBox() {
		JComboBox<T> jcb = new JComboBox<T>();
		jcb.setFont(LABEL_FONT);
		return jcb;
	...
	public static ImageIcon createSmallIcon(String image) {
		ImageIcon icon = null;
		Image i = ResourceLoader.loadResourceImage(image);
		icon = new ImageIcon(i.getScaledInstance(dpi * 16 / 96, dpi * 16 / 96, Image.SCALE_SMOOTH));
		return icon;
	}

	public static ImageIcon createSmallIcon(BufferedImage image) {
		ImageIcon icon = new ImageIcon(image.getScaledInstance(dpi * 16 / 96, dpi * 16 / 96, Image.SCALE_SMOOTH));
		return icon;
	}
}
