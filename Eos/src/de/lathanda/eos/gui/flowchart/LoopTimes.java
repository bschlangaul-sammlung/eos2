...
public class LoopTimes extends ConnectedUnit {
	...
	private final String yes = Text.TEXT.getString("FlowChart.Yes");
	private final String no = Text.TEXT.getString("FlowChart.No");
	...
	LoopTimes(LoopTimesUnit n) {
		...
		var = MessageFormat.format(Text.TEXT.getString("FlowChart.RepeatTimes.Var"), n.getIndexId(), n.getTimes().getLabel());
		diam = new Diamond(MessageFormat.format(Text.TEXT.getString("FlowChart.RepeatTimes.Cond"), n.getIndexId()));
		step = MessageFormat.format(Text.TEXT.getString("FlowChart.RepeatTimes.Step"), n.getIndexId());
	...
	public void layoutUnit(Drawing d) {
		...
		width = (float) Math.max(Math.max(sequence.getWidth(), diam.getWidth()),
				Math.max(inc.getWidth(), init.getWidth())) + 2 * SPACEX;
		center = (width / 2);
