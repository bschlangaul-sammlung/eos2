...
import de.lathanda.eos.baseparser.Format;
...
public class PrintPanel extends javax.swing.JPanel implements Printable, Pageable {
	...
	public PrintPanel() {
		try {
			...
			JOptionPane.showMessageDialog(this, Messages.getString("Print.Error.Title"), t.getLocalizedMessage(),
					JOptionPane.ERROR_MESSAGE);
			pageFormat = new PageFormat();
		}

		Paper paper = pageFormat.getPaper();
		paper.setImageableArea(PAPER_BORDER_X, PAPER_BORDER_Y, paper.getWidth() - 2 * PAPER_BORDER_X,
				paper.getHeight() - 2 * PAPER_BORDER_Y);
	...
	protected void paintComponent(Graphics g) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, Messages.getString("Print.Error.Title"), ex.getLocalizedMessage(),
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void parse(Graphics g) {
		...
		double y = g.getFontMetrics(fontLiteral).getHeight(); // y within actual
																// page
		...
		Line footerLine = new Line(Color.BLACK, offsetX, offsetY + pageheight, offsetX + linewidth,
				offsetY + pageheight);
		int pageNr = 1;
		for (LinkedList<PrintPart> p : pages) {
			Text footerText = new Text(pageNr + "/" + pages.size(), fontFooter, Color.BLACK, false);
			footerText.move(offsetX + (linewidth - footerText.getMetrics(g).getWidth()) / 2,
					offsetY + pageheight + footerHeight);
			pageNr++;
			p.add(headerLine);
			p.add(footerLine);
			p.add(footerText);
