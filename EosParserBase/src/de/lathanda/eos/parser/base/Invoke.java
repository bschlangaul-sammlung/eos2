...
import de.lathanda.eos.parser.core.SystemFunctionType;
...
public class Invoke extends Expression {
	...
	public void resolveNamesAndTypes(Expression with, Environment env) {
		...
		if (target == null) {
			// Systemfunktion
			methodType = SystemFunctionType.getSystemFunction(methodname, arguments.getTypes());
			if (methodType != null) {
				if (!methodType.checkType(arguments.getTypes())) {
					env.addError(marker, "FunctionType", methodType, arguments);
				}
		...
		if (tartype == null) {
			env.addError(marker, "UnknownVariable", target);
		} else {
			methodType = tartype.getMethod(methodname, arguments.getTypes());
		}

		if (methodType == null) {
			env.addError(marker, "UnknownInvoke", methodname, arguments.getTypes().length, tartype);
		} else if (!methodType.checkType(arguments.getTypes())) {
			env.addError(marker, "ParameterType", methodType, arguments);
